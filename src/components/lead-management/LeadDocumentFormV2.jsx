  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <Typography variant="body2">Upload happens after lead creation. <Typography component="span" variant="caption" sx={{ color: 'var(--color-grey-500)' }}>(Max {MAX_FILE_SIZE_LABEL} per file)</Typography></Typography>
        <Grid container spacing={2}>
          {[
            ...
            <Grid item xs={12} sm={6} key={key}>
              <TextField type="file" label={label} fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: IMAGE_ACCEPT, capture: 'environment' }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, [key]: f || null })); }} />
            </Grid>
          ))}
          ...
          <Grid item xs={12} sm={6}>
            <TextField type="file" label="RC Front / Single" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT, capture: 'environment' }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, rcFront: f || null })); }} />
          </Grid>
          {rcPageMode === 'double' && <Grid item xs={12} sm={6}><TextField type="file" label="RC Back" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT, capture: 'environment' }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, rcBack: f || null })); }} /></Grid>}
          <Grid item xs={12} sm={6}>
          ...
          <Grid item xs={12} sm={6}>
            <TextField type="file" label="Aadhaar Front / Single" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT, capture: 'environment' }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, aadhaarFront: f || null })); }} />
          </Grid>
          {aadhaarPageMode === 'double' && <Grid item xs={12} sm={6}><TextField type="file" label="Aadhaar Back" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT, capture: 'environment' }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, aadhaarBack: f || null })); }} /></Grid>}
          ...
          <Grid item xs={12} sm={6}><TextField type="file" label="Bank Proof Upload" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, bankDetail: f || null })); }} /></Grid>
          ...
          <Grid item xs={12} sm={6}><TextField type="file" label="PAN Upload" fullWidth sx={inputSx} InputLabelProps={{ shrink: true }} inputProps={{ accept: DOCUMENT_ACCEPT }} helperText={`Max ${MAX_FILE_SIZE_LABEL}`} onChange={(e) => { const f = e.target.files?.[0]; if (f && !validateFileSize(f)) { e.target.value = ''; return; } setFiles((p) => ({ ...p, pan: f || null })); }} /></Grid>
        </Grid>
      </Box>
    </NormalModal>
  );
