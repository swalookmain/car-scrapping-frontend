...
const StaffTable = ({ isLoading, organizationId = null }) => {
  ...
  const { data: staffResult, isLoading: loadingData, refetch: refetchStaff } = useQuery({
    ...
    queryFn: async () => {
      if (organizationId) {
        ...
        return {
          data: items.map((it) => ({ id: it._id || it.id, name: it.name, phone: it.phoneNumber || it.phone || '', email: it.email || '', status: it.isActive ? 'Active' : 'Inactive', allowedModules: it.allowedModules || [] })),
        ...
        return {
          data: staffItems.map((it) => ({ id: it._id || it.id, name: it.name, phone: it.phoneNumber || it.phone || '', email: it.email || '', status: it.isActive ? 'Active' : 'Inactive', allowedModules: it.allowedModules || [] })),
  ...
  const handleCreateStaff = async (staff) => {
    try {
      const payload = { name: staff.name, phoneNumber: staff.phone, email: staff.email, password: staff.password, isActive: Boolean(staff.isActive), allowedModules: staff.allowedModules || [] };
  ...
  const handleCreateOrUpdateStaff = async (staff, editingId) => {
    if (editingId) {
      try {
        await usersApi.updateUser(editingId, { name: staff.name, phoneNumber: staff.phone, email: staff.email, isActive: Boolean(staff.isActive), allowedModules: staff.allowedModules || [] });
